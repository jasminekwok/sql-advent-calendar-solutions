...
SELECT target_name, prank_id, evilness_score
FROM (
SELECT target_name, prank_id, evilness_score, 
  ROW_NUMBER() OVER(PARTITION BY target_name ORDER BY evilness_score DESC, created_at DESC) AS prank_rank
FROM grinch_prank_ideas) ranked_pranks
WHERE prank_rank = 1;
